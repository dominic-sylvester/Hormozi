...
import { buildAuthorSopMessage } from "../lib/sops";
import { SopPanel } from "./SopPanel";

type OperatingTab = "inbox" | "sops";
...
export function OperatingPanel({ companyId, disabled, onSendMessage }: OperatingPanelProps) {
  const [tab, setTab] = useState<OperatingTab>("inbox");
    ...
    <section className="ops-panel" aria-label="Operating dashboard">
      <div className="ops-panel-header">
        <div>
          ...
          <h2>Action inbox & SOPs</h2>
        </div>
        <div className="ops-panel-actions">
          <div className="ops-tabs" role="tablist" aria-label="Operating views">
            <button
              type="button"
              role="tab"
              aria-selected={tab === "inbox"}
              className={`ops-tab${tab === "inbox" ? " is-active" : ""}`}
            ...
            <button
              type="button"
              role="tab"
              aria-selected={tab === "sops"}
              className={`ops-tab${tab === "sops" ? " is-active" : ""}`}
              onClick={() => setTab("sops")}
            >
              SOPs
            </button>
          ...
          {tab === "inbox" ? (
            ...
            <button
              ...
              className="ghost-button"
              disabled={disabled}
              onClick={() => void onSendMessage(buildAuthorSopMessage())}
            >
              Author with CEO
            </button>
          )}
      ...
      {collapsed ? null : (
        <div className="ops-panel-body">
          {tab === "inbox" ? (
            ...
            <SopPanel companyId={companyId} disabled={disabled} onSendMessage={onSendMessage} />
          )}
        </div>
      )}
    </section>
